...
namespace ae
{
	...
	Image::Image( const Uint32 _Width, const Uint32 _Height, const ImageFormat _Format ) :
		Image()
	{
	...
	Bool Image::LoadFromFile( const std::string& _FileName )
	{
		...
		Uint8* Pixels = priv::STBLoadFileUint8( Width, Height, ChannelsCount, _FileName );
	...
	Bool Image::LoadFromMemory( const Uint8* _Data, const Uint32 _Width, const Uint32 _Height, const ImageFormat _Format )
	{
		if( _Data == nullptr )
		{
	...
	void Image::AllocatePixelsArray( const Uint32 _Width, const Uint32 _Height, const Uint32 _ChannelsCount )
	{
	...
	void Image::CopyPixels( const Uint8* _Pixels )
	{
		std::memcpy( m_Pixels.data(), _Pixels, m_Pixels.size() * sizeof( Uint8 ) );
	}

	Color Image::GetPixelConverted( const Uint32 _Index ) const
	{
		// Get back the pixel fom array.
		const Uint8* PixelPtr = &m_Pixels[Cast( size_t, _Index ) * Cast( size_t, m_ChannelsCount )];
	...
	void Image::SetPixelConverted( const Uint32 _Index, const Color& _Color )
	{
		// Get back the pixel fom array.
		Uint8* PixelPtr = &m_Pixels[Cast( size_t, _Index ) * Cast( size_t, m_ChannelsCount )];
	...
	TexturePixelFormat Image::GetTexturePixelFormat() const
	{
		switch( m_ChannelsCount )
		{
		case 1:
			return TexturePixelFormat::Red_U8;

		case 2:
			return TexturePixelFormat::RedGreen_U8;

		case 3:
			return TexturePixelFormat::RGB_U8;

		case 4:
			return TexturePixelFormat::RGBA_U8;
	...
	{
		Bool Error = priv::STBWriteToPngUint8( _FileName, *this );
	...
	Bool Image::Resize( const Uint32 _NewWidth, const Uint32 _NewHeight )
	{
		Uint8* Resized = Cast( Uint8*, malloc( sizeof( Uint8 ) * _NewWidth * _NewHeight * m_ChannelsCount ) );
