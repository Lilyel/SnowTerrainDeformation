...
#define getcwd _getcwd
    ...
    const char* home_dir()
    {
        ...
        char* user_profile;
        size_t length;
        _dupenv_s( &user_profile, &length, "USERPROFILE" );
    ...
    void suggest_log_path( const char* prefix, char* buff, unsigned buff_size )
    {
        ...
        strncat_s( buff, n - 1, s_argv0_filename.c_str(), buff_size - strlen( buff ) - 1 );
        strncat_s( buff, n - 1, "/", buff_size - strlen( buff ) - 1 );
        write_date_time( buff + strlen( buff ), buff_size - strlen( buff ) );
        strncat_s( buff, n - 1, ".log", buff_size - strlen( buff ) - 1 );
    }
...
#undef DEFINE_EC
        ...
        Text ec_to_text( EcHandle ec_handle )
    {
        ...
        const size_t sizeDest = strlen( parent_ec.c_str() ) + 2;
        char* with_newline = reinterpret_cast<char*>( malloc( sizeDest ) );
        with_newline[0] = '\n';
        strcpy_s( with_newline + 1, sizeDest - 1, parent_ec.c_str() );
